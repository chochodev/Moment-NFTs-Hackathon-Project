...
function mint() {
  ...
  const onSubmit = async (e) => {
    ...
    try {
      ...
      alert(`Minted. Contract : ${contractAddress}, tokenID : ${tokenId}`);

      //Minted Link
      setMintedLink(
        `testnets.opensea.io/assets/rinkeby/${contractAddress}/${tokenId}/?force_update=true`
      );
    ...
    <Fragment>
      ...
      {show ? (
        <div
          id="small-modal"
          tabIndex="-1"
          className="overflow-y-auto overflow-x-hidden fixed mt-24 top-0 right-0 left-0 z-50 w-full md:inset-0 h-modal md:h-full"
        >
                ...
                <form>
                  <div className="relative z-0 mb-6 w-full group">
                    <input
                      type="text"
                      value={name}
                      name="floating_nft-title"
                      id="floating_nft-title"
      ...
      {selectedFile ? (
        <p className="text-center text-lg font-bold text-gray-500 lg:text-xl dark:text-gray-400 mb-6">
          Edit Info and you are ready to Mint!
        </p>
      ) : (
        <>
          <p className="text-center text-lg font-bold text-gray-500 lg:text-xl dark:text-gray-400 mb-6">
            ...
            {selectedFile ? (
              <>
                <button
                  type="button"
                  onClick={openModal}
                  className="text-black bg-cyan-50 font-medium rounded-lg text-md px-14 py-2 text-center mt-2 mr-2"
                >
                  Edit Info
                </button>
                ...
                {name === '' || description === '' ? (
                  ...
                  >
                    <svg
                      role="status"
                      className="mb-1 inline mr-3 w-4 h-4 text-white animate-spin"
                      viewBox="0 0 100 101"
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                    >
